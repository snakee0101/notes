...
class Image extends Model
{
    ...
    public static function processUpload(UploadedFile $image)
    {
        ...
        if($image_width <= 240)
        {
            $thumbnail_small_path = $image_path;
            $thumbnail_large_path = $image_path;
        } elseif (($image_width > 240) && ($image_width <= 600)) {
            $thumbnail_small_path = '/storage/thumbnails_small/' . $filename;
            $thumbnail_large_path = '/storage/thumbnails_small/' . $filename;
            $resource->widen(240)->save($thumbnail_small_path, 100);
        } else {
            $thumbnail_small_path = '/storage/thumbnails_small/' . $filename;
            $thumbnail_large_path = '/storage/thumbnails_large/' . $filename;
            $resource->widen(600)->save($thumbnail_large_path, 100);
            $resource->widen(240)->save($thumbnail_small_path, 100);
        }
