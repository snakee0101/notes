<template>
    <b-modal modal-class="collaborators-dialog" ref="collaborators-dialog" hide-footer centered>
        <div class="collaborators-content">
            <h3 class="font-medium text-lg border-b-2 border-gray-200">Collaborators</h3>

            <div class="collaborator flex flex-row my-4 items-center">
                <img class="collaborator-image"
                     src="data:image/png;base64,">
                <div class="ml-3">
                    <p class="text-sm font-bold">{{ owner.name }}
                        <span class="italic font-normal muted">(Owner)</span>
                    </p>
                    <p class="text-sm muted">{{ owner.email }}</p>
                </div>
            </div>

            <div class="collaborator flex flex-row mb-4 items-center" v-for="collaborator in note.collaborators" :key="collaborator.id">
                <img class="collaborator-image"
                     src="data:image/png;base64,">
                <p class="ml-3 flex-grow text-sm font-bold">{{ collaborator.email }}</p>
                <a href="" class="delete-collaborator p-1"
                   v-b-tooltip.hover.bottom title="Delete"
                   @click.prevent="deleteCollaborator(collaborator)">
                    <i class="bi bi-x icon-sm"></i>
                </a>
            </div>

            <div class="collaborator flex flex-row mb-4 items-center">
                <img class="collaborator-image"
                     src="data:image/png;base64,">
                <input class="add-collaborator-input" type="email"
                       placeholder="Person or email to share with"
                       required v-model="newEmail"
                       :class="errorShown ? 'invalid' : ''"
                       v-on:keyup.enter="checkCollaborator(newEmail)">
                <a href="" class="p-1"
                   v-b-tooltip.hover.bottom title="Add collaborator"
                   @click.prevent="checkCollaborator(newEmail)">
                    <i class="bi bi-check2 icon-sm"></i>
                </a>
            </div>

            <p v-if="errorShown" class="text-sm text-red-700">{{ errorMessage }}</p>
        </div>
        <div class="collaborators-footer">
            <button @click="hide()" class="collaborator-button">Done</button>
        </div>
    </b-modal>
</template>

<script>
export default {
    name: "CollaboratorDialogComponent",
    created() {
        window.events.$on('show-collaborators-dialog', this.show);
    },
    data() {
        return {
            newEmail: '',
            owner: this.$attrs.owner,
            checkingEmail: '',
            note: this.$attrs.note,
            errorShown: false,
            errorMessage: ''
        };
    },
    methods: {
        show(note_id) {
            if (this.note.id === note_id)
                this.$refs['collaborators-dialog'].show();
        },
        hide() {
            this.$refs['collaborators-dialog'].hide();
        },
        showError(message) {
            this.errorMessage = message;
            this.errorShown = true;

            clearTimeout(window.collaboratorErrorTimeoutId);
            window.collaboratorErrorTimeoutId = setTimeout(this.hideError, 3000);
        },
        hideError() {
            this.errorShown = false;
        },
        deleteCollaborator(user) {
            this.note.collaborators = _.without(this.note.collaborators, user);
            this.confirm();
        },
        checkCollaborator(email) {
            this.checkingEmail = email;

            axios.get('/collaborator/' + email)
                .then(this.checkUserExistence);
        },
        checkUserExistence(response) {
            if (response.data.exists === false)
                return this.showError("The user with requested email is not registered");

            if (this.owner.email === this.checkingEmail)
                return this.showError("You are already the owner of the note");

            if (this.note.collaborators.includes(response.data.user))
                return this.showError("The user you want to add is already your collaborator");

            this.addCollaborator(response.data.user);
        },
        addCollaborator(user) {
            this.note.collaborators.push(user);
            this.newEmail = '';

            this.confirm();
        },
        confirm() {
            if(this.note.id == 'new_note') {
                window.events.$emit('save_new_note_collaborators', this.note.collaborators);
            } else {
                axios.post('/collaborator/' + this.note.id, {
                    emails: this.note.collaborators.map( user => user.email )
                }).then(res => this.note.collaborators = res.data);
            }
        },
    }
}
</script>

<style scoped>

</style>
